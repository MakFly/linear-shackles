...
const normalizeRepo = (repoInput: string): string => {
  ...
  const githubUrlMatch = repoInput.match(/github\.com[/:]([^/]+)\/([^/]+?)(?:\.git|\/|$)/);
  if (githubUrlMatch) {
...
export const useGitHub = () => {
  ...
  const getUserRepositories = async () => {
    ...
    while (true) {
      const response = await fetch(`${GITHUB_API_BASE}/user/repos?per_page=${perPage}&page=${page}&sort=updated`, {
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/vnd.github+json",
          "X-GitHub-Api-Version": "2022-11-28",
  ...
  const getPullRequests = async (state: "open" | "closed" | "all" = "open") => {
    return makeRequest<GitHubPullRequest[]>(`/pulls?state=${state}&per_page=100`);
  };
  ...
  const createIssue = async (title: string, body?: string, labels?: string[]) => {
