...
interface UseWorkflowMonitorOptions {
  ...
  onWorkflowChange?: (workflow: GitHubWorkflowRun, previousState: string, newState: string) => void;
}
...
export const useGitHubWorkflowMonitor = (
  ...
  const getWorkflowStateKey = useCallback((workflow: GitHubWorkflowRun): string => {
  ...
  const checkWorkflowChanges = useCallback(async () => {
    ...
    try {
      ...
      const currentWorkflowsMap = new Map(currentWorkflows.map((w) => [w.id, w]));
      ...
      for (const currentWorkflow of currentWorkflows) {
        ...
        if (previousWorkflow) {
          ...
          if (previousState !== currentState) {
            const stateChanged = previousWorkflow.status !== currentWorkflow.status;

            // Notifier les changements importants
            if (stateChanged) {
              if (previousWorkflow.status === "queued" && currentWorkflow.status === "in_progress") {
                toast.info(`🚀 Workflow démarré`, {
                  description: `${currentWorkflow.name} a commencé à s'exécuter`,
                });
              } else if (previousWorkflow.status === "in_progress" && currentWorkflow.status === "completed") {
                if (currentWorkflow.conclusion === "success") {
                  toast.success(`✅ Workflow réussi`, {
                    ...
                    action: {
                      label: "Voir",
                      onClick: () => window.open(currentWorkflow.html_url, "_blank"),
                    },
                  });
          ...
          if (currentWorkflow.status === "queued" || currentWorkflow.status === "in_progress") {
