...
function ProjectGitHub() {
  const { projectId } = useParams({ from: "/projects/$projectId/github" });
  const { isConnected, repo, connect, disconnect, getIssues, getPullRequests, getBranches, getWorkflowRuns, createIssue } = useGitHub();
  const { toast } = useToast();
  
  const [showSettings, setShowSettings] = useState(!isConnected);
  ...
  const handleConnect = () => {
    if (!tokenInput || !repoInput) {
      toast({
        title: "Erreur",
        description: "Token et repository requis",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleCreateIssue = async () => {
    ...
    try {
      ...
      toast({
        title: "Issue créée",
        description: "L'issue a été créée avec succès sur GitHub",
      });
    ...
    return (
      <div className="flex-1 p-8 overflow-auto">
        <Card className="max-w-2xl mx-auto">
          <CardHeader>
            <CardTitle>Configuration GitHub - {projectId}</CardTitle>
            <CardDescription>
              Connectez votre repository GitHub pour synchroniser les issues, PRs, CI/CD et branches
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <Alert>
              ...
              <AlertDescription>
                <strong>Important :</strong> Votre token sera stocké localement dans le navigateur.
                Pour une sécurité optimale, utilisez un Personal Access Token avec des permissions limitées.
              </AlertDescription>
            </Alert>
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-sm text-muted-foreground">
                Format: owner/repo (ex: facebook/react)
              </p>
            </div>
    ...
    <div className="flex-1 p-8 overflow-auto">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold">GitHub Integration - {projectId}</h1>
            <p className="text-muted-foreground mt-1">
              Repository: <span className="font-mono">{repo || "Non connecté"}</span>
            </p>
          </div>
          <div className="flex gap-2">
          ...
          <Card>
            <CardContent className="p-6 text-center">
              <p className="text-muted-foreground mb-4">
                Connectez votre repository GitHub pour commencer
              </p>
