...
function Component() {
  ...
  const getUpdateIcon = (type: string) => {
    switch (type) {
      ...
      case "sprint_created": return Calendar;
  ...
  const getStatusColor = (status?: string) => {
    switch (status) {
      case "done": return "bg-[hsl(var(--status-done))]/10 text-[hsl(var(--status-done))] border-[hsl(var(--status-done))]/20";
      case "progress": return "bg-[hsl(var(--status-progress))]/10 text-[hsl(var(--status-progress))] border-[hsl(var(--status-progress))]/20";
      case "warning": return "bg-[hsl(var(--status-warning))]/10 text-[hsl(var(--status-warning))] border-[hsl(var(--status-warning))]/20";
      case "backlog": return "bg-muted text-muted-foreground border-muted";
  ...
  const getUpdateTypeLabel = (type: string) => {
    switch (type) {
      ...
      case "sprint_created": return "Sprint créé";
      case "issue_created": return "Issue créée";
    ...
    if (searchQuery) {
      filtered = filtered.filter((update) => {
        ...
        return (
          ...
          (metadata?.issueTitle as string)?.toLowerCase().includes(searchQuery.toLowerCase()) ||
          (metadata?.sprintName as string)?.toLowerCase().includes(searchQuery.toLowerCase())
        );
      });
    }
  ...
  const stats = useMemo(() => {
    ...
    const statusChanges = dbUpdates.filter((u) => u.type === "status_change").length;
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl font-bold text-foreground">Activité du projet</h1>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button onClick={() => setShowNewUpdate(!showNewUpdate)} className="gap-2">
            ...
            <Card className="border-border bg-card/50 backdrop-blur">
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Mises à jour aujourd'hui</p>
                </div>
              </CardContent>
            </Card>
            <Card className="border-border bg-card/50 backdrop-blur">
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-foreground">{stats.statusChanges}</p>
                  <p className="text-xs text-muted-foreground">Changements de statut</p>
                </div>
              </CardContent>
            </Card>
            <Card className="border-border bg-card/50 backdrop-blur">
        ...
        {showNewUpdate && (
          <Card className="border-border bg-card animate-fade-in">
            <CardHeader>
              <CardTitle className="text-card-foreground">Publier une mise à jour</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={() => { setShowNewUpdate(false); setNewUpdate(""); }}>
                  Annuler
                </Button>
                <Button onClick={() => { setNewUpdate(""); setShowNewUpdate(false); toast.success("Update publié (sera ajouté en DB)"); }}>
                  Publier
                </Button>
              </div>
            </CardContent>
            ...
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <Tabs value={filterType} onValueChange={setFilterType} className="w-full md:w-auto">
                <TabsList className="grid grid-cols-4 md:grid-cols-7 w-full">
                  ...
                  <TabsTrigger value="status_change" className="text-xs">Statuts</TabsTrigger>
                  <TabsTrigger value="sprint_created" className="text-xs">Sprints</TabsTrigger>
                  <TabsTrigger value="issue_created" className="text-xs">Issues</TabsTrigger>
                  <TabsTrigger value="comment" className="text-xs">Comments</TabsTrigger>
                  <TabsTrigger value="relationship" className="text-xs">Relations</TabsTrigger>
                  <TabsTrigger value="review" className="text-xs">Revues</TabsTrigger>
                </TabsList>
              </Tabs>
            </div>
          </CardContent>
          ...
          {groupedUpdates.length === 0 ? (
            <Card className="border-border bg-card/50">
              <CardContent className="p-12 text-center">
                ...
                <p className="text-muted-foreground">Aucune mise à jour trouvée</p>
              </CardContent>
            </Card>
          ) : (
                        ...
                        <CardContent className="p-4">
                          <div className="flex gap-4">
                            <div className="flex flex-col items-center">
                              <div className={`p-2 rounded-lg border ${getUpdateColor(update.type)} transition-transform group-hover:scale-110`}>
                            ...
                            <div className="flex-1 min-w-0 space-y-2">
                              ...
                              {update.type === "status_change" && metadata && (
                                <div className="space-y-2">
                                  <p className="text-sm text-muted-foreground">
                                    {update.content}{" "}
                                    <span className="font-medium text-foreground">{metadata.issueTitle as string}</span>
                                  </p>
                                  <div className="flex items-center gap-2 flex-wrap">
                                    <Badge variant="outline" className={`${getStatusColor(metadata.oldStatus as string)} text-xs`}>
                                      {metadata.oldStatus as string}
                                    </Badge>
                                    <ArrowRight className="h-3 w-3 text-muted-foreground" />
                                    <Badge variant="outline" className={`${getStatusColor(metadata.newStatus as string)} text-xs`}>
                              ...
                              {update.type === "sprint_created" && metadata && (
                                <div className="space-y-2">
                                  ...
                                  <div className="flex items-center gap-2 p-2 bg-primary/5 rounded-lg border border-primary/10">
                                    ...
                                    <span className="text-sm font-medium text-foreground">{metadata.sprintName as string}</span>
                                  </div>
                                </div>
                              )}
                              ...
                              {update.type === "issue_created" && metadata && (
                                <div className="space-y-2">
                                  <p className="text-sm text-muted-foreground">{update.content}</p>
                                  <div className="flex items-center gap-2 flex-wrap">
                                    <span className="text-sm font-medium text-foreground">{metadata.issueTitle as string}</span>
                                    {metadata.priority && (
                                      <Badge
                                        variant="outline"
                                        className={`text-xs ${
                              ...
                              {update.type === "comment" && metadata && (
                                <div className="space-y-2">
                                  ...
                                  <div className="p-3 bg-muted/50 rounded-lg border border-border">
                                    <p className="text-sm text-foreground italic">{update.content}</p>
                                  </div>
                                </div>
                              )}

                              {(update.type === "relationship" || update.type === "review") && metadata && (
                                <div className="space-y-1">
                                  <p className="text-sm text-muted-foreground">
                                    {update.content}{" "}
                                    <span className="font-medium text-foreground">
                                      {(metadata.issueTitle || metadata.sprintName) as string}
                                    </span>
                                  </p>
                                </div>
                              )}
