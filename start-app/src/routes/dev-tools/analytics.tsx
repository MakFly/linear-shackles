...
import { TrendingUp, TrendingDown, CheckCircle2, Clock, AlertTriangle, Target, Database, RefreshCw } from "lucide-react";
import { getMockAnalyticsData, getMockIssues, getMockSprints } from "@/server/dev-tools-mocks";
...
function Component() {
  ...
  const issuesByStatusData = [
    { name: "Terminé", value: issuesByStatus.done, color: "hsl(var(--status-done))" },
    { name: "En cours", value: issuesByStatus.progress, color: "hsl(var(--status-progress))" },
    { name: "En attente", value: issuesByStatus.warning, color: "hsl(var(--status-warning))" },
    { name: "Backlog", value: issuesByStatus.backlog, color: "hsl(var(--status-backlog))" },
  ];

  // Generate velocity data from sprints
  const velocityData = sprints.slice(0, 5).map((sprint, i) => {
  ...
  const issuesTrend = [
    { date: "Lun", created: Math.floor(totalIssues * 0.1), resolved: Math.floor(issuesByStatus.done * 0.15) },
    { date: "Mar", created: Math.floor(totalIssues * 0.15), resolved: Math.floor(issuesByStatus.done * 0.2) },
    { date: "Mer", created: Math.floor(totalIssues * 0.08), resolved: Math.floor(issuesByStatus.done * 0.25) },
    { date: "Jeu", created: Math.floor(totalIssues * 0.12), resolved: Math.floor(issuesByStatus.done * 0.18) },
    { date: "Ven", created: Math.floor(totalIssues * 0.06), resolved: Math.floor(issuesByStatus.done * 0.22) },
  ];
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Répartition actuelle des issues (DB)</CardDescription>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Issues créées vs résolues (estimé)</CardDescription>
                </CardHeader>
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
          ...
          <TabsContent value="velocity" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Points planifiés vs complétés (données réelles)</CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <TabsContent value="trends" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Évolution sur la semaine (estimé)</CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
